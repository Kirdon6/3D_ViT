...
def main(args: argparse.Namespace):
    ...
    embeddings = Embedder(args.embd_size)(inputs)
    
    transformer_output = ViT3D(args.num_heads, args.embd_size + CATEGORICAL_F_SIZE + NEIGHBOR_F_SIZE + COORDINATES_SIZE,args.num_layers,args.key_dim, args.val_dim)(embeddings)
    predictions = tf.keras.layers.Dense(1, activation=None)(transformer_output)
    
    model = tf.keras.Model(inputs=inputs, outputs=predictions)
    optimizer = tf.optimizers.experimental.AdamW(learning_rate=args.learning_rate)
